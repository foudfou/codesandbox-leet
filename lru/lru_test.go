...
func TestLRUCache(t *testing.T) {
	t.Run("lru", func(t *testing.T) {
		...
		v, _ := lru.Get(1)
		AssertEqual(t, v, 1) // returns 1
		AssertEqual(t, lru.String(), "[1:1,2:2]")
		lru.Put(3, 3) // evicts key 2
		AssertEqual(t, lru.String(), "[3:3,1:1]")
		v, err := lru.Get(2)
		AssertEqual(t, err, ErrNotFound) // returns -1 (not found)
		AssertEqual(t, v, 0)             // actually returns null value
		lru.Put(4, 4)                    // evicts key 1
		AssertEqual(t, lru.String(), "[4:4,3:3]")
		v, err = lru.Get(1)
		AssertEqual(t, err, ErrNotFound) // returns -1 (not found)
		v, _ = lru.Get(3)
		AssertEqual(t, v, 3) // returns 3
		AssertEqual(t, lru.String(), "[3:3,4:4]")
		v, _ = lru.Get(4)
		AssertEqual(t, v, 4) // returns 4
		AssertEqual(t, lru.String(), "[4:4,3:3]")
	})
